...
# serv = 'SQLREPORTING\\SQLREPORTING'
# database = 'jobdivaapi'
...
"""
#### manual run ###########
metric = 'New/Updated Job Records' #" #"New/Updated Contact Records" #"Divisions List" #
days_back =7
iterations = 1

# def server to get metric params and save api return data:  
server = '(localdb)\\MSSQLLocalDB'
serv = '(localdb)\MSSQLLocalDB'
database = 'jobdivaapi'
driver = '{ODBC Driver 17 for SQL Server}'
"""
...
def get_metric_info():
    ...
    query_string_report = "Select schema_name, MetricName, UDF, UDF2,UDF3, UDF4, UDF5, pr from JobDivaAPI.API.Metrics where MetricName ='" + metric + "'"
...
# iterations = how many blocks of 14 days in the past the data should return
        ...
        for call in tree.iter('Data'):
            for record in call.iter('Columns'):
                ...
                for val in record.iter('Column'):
                    ...
                    # print(all_columns)
                col_query = f"select column_name from INFORMATION_SCHEMA.COLUMNS  where TABLE_NAME='{TableName}' and TABLE_SCHEMA='{schema}'"
                ...
                try:
                    if len(db_create_col) > 0:
                        for c in db_create_col:
                            ...
                            col_log_query = f"insert into JobDivaAPI.api.added_columns (column_added,tablename) values ('{c}','{table}')"
                            # print(col_log_query)
                            cursor.execute(col_log_query)
                            cursor.commit()
                except Exception as e:
                    error_add_col = f"insert into JobDivaAPI.api.added_columns (column_added,tablename) values ('{c}','{table},'{e}')"
            ...
            if run_lines == 0:
                ...
                if run_lines == 1:
                    ...
                    try:
                        ...
                        error_string = "INSERT INTO [JobDivaAPI].[API].[ERROR_LOG] ([metric] ,[query] ,[query_val],[error_txt])  VALUES (%s)" % error_val_string
...
# insert data into sql database, check for errors
    try:
        ...
        error_list.append(MetricName)
        error_list.append(str(e))
        error_string = "INSERT INTO [JobDivaAPI].[API].[ERROR_LOG] ([metric], [error_txt])  VALUES (%s)" % error_val_string
        cursor.execute(error_string, error_list)
